...
export class FileSystemProvider implements vscode.TreeDataProvider<Entry>, vscode.FileSystemProvider {
	...
	async getChildren(element?: Entry): Promise<Entry[]> {
		return [{ uri: vscode.Uri.file(path.join("", "测试1")),type:2 }]
		// if (element) {
	...
	getTreeItem(element: Entry): vscode.TreeItem {
		...
		if (element.type === vscode.FileType.File) {
			treeItem.command = { command: 'gitBranch.openFile', title: "Open File", arguments: [element.uri], };
...
export class FileExplorer {
	...
	constructor(context: vscode.ExtensionContext) {
		...
		vscode.commands.registerCommand('gitBranch.openFile', (resource) => this.openResource(resource));
	}
